...
via yaw/pitch angles, producing a dense H×W range image.
"""
...
class RangeProjection:
    def __init__(self, proj_h=64, proj_w=512, fov_up=3.0, fov_down=-25.0):
        """
        Args:
            proj_h / proj_w : range image dimensions (64×512 for SemanticKITTI)
        ...
        self.cached   = {}  # stores back-projection indices after project()
    ...
    def project(self, points):
        ...
        px = np.floor(u * W).clip(0, W - 1).astype(np.int32)
        py = np.floor(v * H).clip(0, H - 1).astype(np.int32)
        ...
        # Cache pixel coordinates for back-projection at inference
        self.cached = {'uproj_x': px, 'uproj_y': py}

        return proj_range, proj_feat, proj_idx, proj_range > 0
