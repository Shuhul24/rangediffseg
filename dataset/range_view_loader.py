...
augmentation, projects them to range-view images and returns the tensors the
segmentation network is trained on.
...
64 x 384 windows of a 64 x 2048 image); at evaluation time the full-width image
is returned and the model is applied with a sliding window.
"""
...
class RangeViewDataset(Dataset):
    def __init__(self, root: str, sequences: list,
                 ...
                 return_points: bool = False, has_label: bool = True):
        """
        Args:
            ...
            has_label     : False for the unlabelled test sequences
        """
        self.parser = SemanticKITTI(root, sequences, has_label=has_label)
        ...
        self.mapped_cls_name = self.parser.mapped_cls_name
        self.cls_freq = self.parser.cls_freq
